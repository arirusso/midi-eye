...
== Usage
  ...
  #
  # in this example, we will tell it to look for note on/off messages
  #
  ...
  #
  # you can bind as many events to a listener as you wish, just keep calling listen_for
  #
  ...
  #
  # or have the listener run only in a background thread by using :background => true
  # this will allow you to run multiple listeners at the same time, ie if you're
...
see another example that deals with MIDI clock sync {here}[http://github.com/arirusso/midi-eye/blob/master/examples/arpeggios.rb]  
